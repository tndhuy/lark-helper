 ...
 */
export async function upsertDocument(doc: IndexDocument) {
 ...
 */
export async function searchKB(query: string, limit: number = 5) {
 ...
 */
export async function indexWikiSpace(spaceId: string) {
  console.log(`🚀 Indexing Wiki Space: ${spaceId}`);
  
  // List all nodes in the space
  const response: any = await runLarkCommand(["wiki", "+nodes", "--space_id", spaceId]);
