...
export function registerIMTools(server: McpServer) {
  server.tool(
    "im_send_message",
    "Send a message to a user or a chat.",
    {
    ...
    async (args) => {
      try {
        ...
        if (args.receive_id_type) cmdArgs.push("--receive_id_type", args.receive_id_type);
        if (args.receive_id) cmdArgs.push("--receive_id", args.receive_id);
        if (args.content) cmdArgs.push("--content", args.content);
        if (args.msg_type) cmdArgs.push("--msg_type", args.msg_type);
        if (args.as_identity) cmdArgs.push("--as_identity", args.as_identity);
  ...
  server.tool(
    "im_list_messages",
    "List messages in a chat or a conversation.",
    {
    ...
    async (args) => {
      try {
        ...
        if (args.container_id_type) cmdArgs.push("--container_id_type", args.container_id_type);
        if (args.container_id) cmdArgs.push("--container_id", args.container_id);
        if (args.as_identity) cmdArgs.push("--as_identity", args.as_identity);
