...
export function registerDocsTools(server) {
    server.tool("docs_fetch", "Fetch content from a Lark document by URL or ID. Returns document content in Markdown or raw format.", {
    ...
    server.tool("docs_search", "Search for Lark documents, wikis, and sheets using keywords and filters.", {
    ...
    server.tool("docs_create", "Create a new Lark document with a title and Markdown content.", {
        ...
        try {
            ...
            if (folder_token)
                args.push("--folder-token", folder_token);
            if (wiki_space)
                args.push("--wiki-space", wiki_space);
            if (wiki_node)
                args.push("--wiki-node", wiki_node);
            const response = await runLarkCommand(args);
            return {
                content: [
                    {
